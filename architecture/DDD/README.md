...
**例：メールアドレス更新時のメールアドレス重複確認**
指定されたメールアドレスがすでに使われているかを調べる時に、ユーザーオブジェクトだけに担わせるのは無理がある。他のユーザーオブジェクトについての状況を知っていないといけないため。

[ドメインサービスの例](./DomainService.ts)
...
| データベース   | 一貫性のあるデータモデルを使用する                   | 複数のデータモデルを使用し、読み取りと書き込みを分離する           |
| メリット       | 複雑性を低減し、単一責任を持ったクラスを作成できる   | 書き込みと読み取りの処理を最適化し、スケーラビリティを向上させる   |
...
特に、ネットワークを介して異なるコンピュータ間でデータを要求したり提供する際に、個々のデータをばらばらに受け渡すよりも DTO の形でまとめて転送する方が伝送効率がよく、データの要求や取得に関連するコードもシンプルに記述しやすくなる。
