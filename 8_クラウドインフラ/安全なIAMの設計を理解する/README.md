...
例: S3バケット(test-bucket)にリソースベースのIAMポリシーを付与し、バケットに対して”123456789123”AWSアカウントの”user”ユーザーがアップロードできる権限を付与
...
- 他のアカウントや外部のプリンシパル、あるいはアカウント内外の特定のエンティティにリソースへのアクセスを許可する場合 → Resource basedポリシー
...
IPアドレスなどを指定することで、アクセスを制限する方式。セキュアだが、管理がやや大変

**ブラックリスト方式**

特定のIPアドレスや特定の地域からのアクセスを拒否する方式。攻撃元がわかっている場合に有効だが、イタチごっこになりうる。

### AWSによるリソースの自動チェック

リソースの設定変更を記録・評価し、セキュリティやコンプライアンスに違反した設定を特定・修復するためのサービス。  
特定のルール違反が発生した際、自動修復アクションをトリガーし、設定を望ましい状態に戻すことが可能。  
AWS Configを活用することで、環境の一貫性とセキュリティの向上が図れる。

## 課題3

Q1. 

複数のAWSリソースへアクセスを行うEC2インスタンス上のアプリケーションを作成している。  
アプリケーションがAWSリソースへアクセスする際は認証が安全に行われる必要がある。  
要件を満たすためにどのような作業が必要か？

必要な権限を付与したIAMポリシーを作成し、IAMロールにアタッチする。IAMロールをEC2インスタンスへ関連付ける

Q2.

複数のAWSアカウントを管理している組織において、チームごとに利用可能なサービスやアクションの制限をしたい。  
もっとも適切なアプローチはどのようなものであるか？

A. OU（Organizational Unit）単位にサービスコントロールポリシー（SCP）を設定する

「AWS Organizations」は複数のAWSアカウントを一元的に管理する機能。AWS Organizationsでは、管理するAWSアカウントに対して権限を一括して設定したり、AWSアカウントごとの請求情報をまとめたりできる。

サービスコントロールポリシー（SCP）は、組織に属しているOU（Organizational Unit）またはAWSアカウントに対して、AWSサービスへのアクセス権限や利用可能なリソースを制限できる機能。  
例えば、組織に所属しているアカウントにMFA（多要素認証）を強制したり、特定のアカウントが利用できるサービスを制限したりできます。
