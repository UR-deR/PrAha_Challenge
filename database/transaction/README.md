...
複数クライアントからの操作がありうるアプリケーションにおいて、テーブル全体をロックしてしまうと、スループットが悪化し UX 面での悪影響があると考えられる。  
したがって、テーブル全体をロックするのではなく、parents テーブル の特定の行の排他ロックを取得するやり方の方が良さそう。

### 課題 3

## クイズ 1

- PostgreSQL における「current_timestamp」と「clock_timestamp」の違いは何でしょうか？（トランザクション内での実行を想定。）
