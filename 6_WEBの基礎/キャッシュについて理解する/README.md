...
**機密情報・個人情報を含むページ**
- CDN 使用時・設定変更時にキャッシュ流出する事例がある。
- あるユーザーキャッシュが別のユーザーに表示されている

[参考:CDN切り替え作業における、Web版メルカリの個人情報流出の原因につきまして | メルカリエンジニアリング](https://engineering.mercari.com/blog/entry/2017-06-22-204500/)

**変更頻度の高いリアルタイム性の求められるページ**

高頻度で更新されるため、キャッシュがすぐに古くなってしまい、リアルタイム性を担保できない。
...
<summery>
リソースが最後に変更された日時を示し、ブラウザが次回リクエスト時にサーバーに変更を確認するために使用されるレスポンスヘッダーは次のうちどれでしょうか？
<details>
Last-Modifiedヘッダーは、リソースが最後に変更された日時を示し、If-Modified-Sinceと共に使用される。
</details>
</summer>
...
<summery>
Cache-Controlヘッダーのpublicとprivateの違いはなんでしょう？
<details>
publicは、リソースがブラウザのキャッシュやプロキシサーバーのキャッシュ、CDN（コンテンツ配信ネットワーク）など、どのキャッシュにも保存されることを許可する。

privateは、リソースがブラウザのキャッシュにのみ保存され、共有キャッシュ（プロキシサーバーやCDNなど）には保存されないことを指定する。
</details>
</summer>
